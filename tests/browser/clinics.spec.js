...
const clinic = {
  ...
  status: "scheduled", professionalAvailable: true, mockPayments: true, registration: null,
};
...
async function account(page, role, currentClinic = { ...clinic }) {
  ...
  const workspace = { role, professionalId: role === "doctor" || role === "lawyer" ? "professional" : null,
    ...
    patient: { id: "patient", name: "Test Patient", status: "active" }, patients: [], sessions: [], bookings: [], weeklyAvailability: [], onboarding: null };
...
test("patient consents, reserves, confirms test payment and cannot join before start", async ({ page }) => {
  ...
  let paid = false;
  await page.route("**/api/clinics/clinic/payment-simulation", (route) => {
    expect(route.request().postDataJSON()).toEqual({ success: true });
    paid = true; current.registration.status = "confirmed"; current.registration.payment = "paid (mock)";
    return route.fulfill({ json: { message: "Clinic payment confirmed." } });
  });
  ...
  await page.getByRole("dialog").getByRole("button", { name: "OK" }).click();
  await page.getByRole("button", { name: "Pay for clinic (test)" }).click();
...
test("admin reviews clinic payments and explicitly confirms a test refund", async ({ page }) => {
  ...
  let refunded = false;
  await page.route("**/api/clinics/clinic/registrations/reg/refund-simulation", (route) => {
    refunded = true;
    return route.fulfill({ json: { message: "Test refund recorded." } });
  });
  await page.goto("/app/clinics/clinic");
  ...
  await page.getByRole("dialog").getByRole("button", { name: "OK" }).click();
  await expect(page.getByText("Test refund recorded.", { exact: true })).toBeVisible();
