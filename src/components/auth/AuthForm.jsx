  ...
  return <PlatformLayout>
    <div className="grid gap-12 lg:grid-cols-2 items-start">
      ...
      <section className="border rounded-2xl p-6 sm:p-9 max-w-lg w-full" style={{ borderColor: "var(--cc-border)", background: "var(--cc-background)" }}>
        ...
        {created ? <div role="status"><p className="my-6">Your registration was submitted. Follow any verification instructions sent by the server, then sign in. Professional accounts require administrator approval.</p><Link className="cc-button" to={`/login${suffix}`}>Continue to login</Link></div> : <form onSubmit={submit} className="space-y-5 mt-7">
          ...
          <Input label="Password" type="password" name="password" value={form.password} onChange={change} autoComplete={signup ? "new-password" : "current-password"} minLength={signup ? 8 : undefined} required disabled={busy} />
          ...
          {!signup && <Link to="/forgot-password" className="inline-block text-sm underline">Forgot your password?</Link>}
