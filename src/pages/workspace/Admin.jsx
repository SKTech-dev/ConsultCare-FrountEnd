...
export function AdminPeople() {
  ...
  return <><PageHeading title="People behind the platform.">Review qualifications, approve professionals, and manage account access.</PageHeading><div className="workspace-sections">{verificationCard("doctor", "Doctor verification", "Registered doctors will appear here for review.")}{verificationCard("lawyer", "Lawyer verification", "Registered lawyers will appear here for review.")}<Panel title="Patient / client accounts">{!s.patients.length && <Empty title="No patient / client accounts">Registered patients and clients will appear here.</Empty>}{s.patients.map((patient) => <div className="ws-row" key={patient.id}><div><Link className="ws-name-link" to={`/app/admin/person/patient/${patient.id}`}>{patient.name}</Link><p>{patient.email}</p></div><Status>{patient.status}</Status><button className="ws-link secondary" onClick={() => setPending({ id: patient.id, status: patient.status === "active" ? "suspended" : "active", name: patient.name })}>{patient.status === "active" ? "Suspend account" : "Restore account"}</button></div>)}</Panel></div><ModerationPopups pending={pending} error={error} submitting={submitting} onClose={() => setPending(null)} onConfirm={confirmModeration} onDismissError={() => setError(null)} /></>;
}
