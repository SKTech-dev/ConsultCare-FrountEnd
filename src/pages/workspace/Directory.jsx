...
import { book, saveFamily } from "../../features/consultations/consultationSlice";
...
export function Directory({ profession }) {
  ...
  const publicSessions = s.sessions.filter((session) => !session.privateAppointment && !session.clinicBlocked);
  ...
  return <>
    ...
    <div className="workspace-sections"><Panel title={profession === "doctor" ? "Doctor consultations" : "Lawyer consultations"}>
    ...
    <ClinicList embedded view="available" profession={profession} title={profession === "doctor" ? "Available doctor clinics" : "Available lawyer clinics"} />
    </div>
  </>;
}
...
export function ProfessionalDetails() {
  ...
  const sessions = sortUpcomingSessions(s.sessions.filter((x) => x.professionalId === id && !x.privateAppointment && !x.clinicBlocked));
