...
import { isSessionLive, queueFor, sessionLabel, sortUpcomingSessions, sriLankanDate } from "../../features/consultations/model";
...
export function Queue() {
  ...
  return <><PageHeading title="Your consultation queues." action={<Link className="ws-link secondary" to="/app/sessions">Manage weekly schedule</Link>}>Each session has its own queue. Showing your scheduled sessions for the next seven days.</PageHeading>
    ...
    {sessions.length ? <div className="queue-sessions">{sessions.map((session) => {
      ...
      const busy = s.bookings.some((booking) => booking.professionalId === p.id && booking.status === "IN CONSULTATION");
      return <Panel key={session.id} title={sessionLabel(session)}><div className="queue-session-heading"><p>{queue.length ? `${queue.length} active patient${queue.length === 1 ? "" : "s"} / client${queue.length === 1 ? "" : "s"} in this session queue.` : "No active bookings for this session yet."}</p><Status>{session.online ? "Available" : "Offline"}</Status></div>{queue.length ? queue.map((booking) => <div className="ws-row" key={booking.id}><div><h3>{booking.position}. {booking.patientName}</h3><p>{booking.payment}</p></div><Status>{booking.status}</Status><div className="ws-actions">{booking.status === "IN CONSULTATION" ? <Link to={`/app/room/${booking.id}`} className="ws-link">Open room</Link> : <><button className="ws-link" disabled={p.status !== "verified" || busy || !session.online || !isSessionLive(session) || booking.status !== "NEXT"} onClick={() => dispatch(transition({ id: booking.id, status: "IN CONSULTATION" }))}>Call next</button><button className="ws-link secondary" disabled={p.status !== "verified"} onClick={() => setSkip(booking.id)}>No-show</button></>}<Link className="ws-link secondary" to={`/app/booking/${booking.id}`}>Details</Link></div></div>) : <div className="queue-empty">This queue is clear.</div>}</Panel>;
