...
export default function Overview() {
  ...
  return <>
    ...
    <div className="ws-grid">{[
      ...
      [s.role === "admin" ? "Awaiting verification" : "Upcoming sessions", s.role === "admin" ? s.professionals.filter((p) => p.status === "pending").length : s.role === "user" ? mine.filter((b) => b.status === "PAYMENT PENDING").length : s.sessions.filter((x) => x.professionalId === s.professionalId).length, s.role === "user" ? "Awaiting your mock payment" : "Stay on top of the next step"],
