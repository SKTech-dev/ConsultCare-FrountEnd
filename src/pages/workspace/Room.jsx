...
export function Documents({ booking }) {
  ...
  return <Panel title="Reports, images & documents">
    ...
    {s.role !== "user" && ACTIVE.includes(booking.status) && <label className="ws-field ws-space"><input type="checkbox" checked={privateNote} onChange={(event) => setPrivateNote(event.target.checked)} /> Attach to private professional notes</label>}
    {ACTIVE.includes(booking.status) && <label className="ws-field ws-space">{busy ? "Saving document…" : s.role === "user" ? "Upload a report or image" : "Attach a note image or advice document"}<input aria-label="Upload consultation document" type="file" disabled={busy} accept=".pdf,.jpg,.jpeg,.png,.webp" onChange={upload} className="block mt-3 text-xs w-full" /></label>}
