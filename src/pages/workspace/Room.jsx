...
import { attach, message, saveNotes, transition } from "../../features/consultations/consultationSlice";
...
export function Documents({ booking }) {
  ...
  async function upload(event) {
    ...
    try {
      ...
      dispatch(attach({ id: booking.id, file: { id, name: file.name, size: file.size, type: file.type } }));
    } catch (failure) { setError(failure.message); }
    finally { setBusy(false); }
  }
  return <Panel title={s.role === "user" ? "Reports & professional documents" : "Documents & prescription upload"}>
    <p>Preview files stay in this browser. Use sample documents only.</p>
...
export default function Room() {
  ...
  async function startCamera() {
    ...
    } catch (error) { if (mounted.current) setMediaError(error.name === "NotAllowedError" ? "Camera or microphone permission was denied. You can continue with the local chat." : error.message); }
    finally { if (mounted.current) setStarting(false); }
  }
  function storeNotes() { dispatch(saveNotes({ id, notes, privateNotes, followUp })); }
