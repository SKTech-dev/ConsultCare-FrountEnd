...
export default function ScheduleConsultation() {
  ...
  async function schedule(event) {
    ...
    setBusy(true);
    try {
      const result = await callApi("POST", "/scheduled-consultations", { email: patient.email, date, start, end, fee: Number(fee), reason: reason.trim() });
      setPatient(null); setEmail(""); setReason(""); setStart(""); setEnd(""); setFee("");
  ...
  return <Panel title="Schedule a patient consultation">
    ...
    <form onSubmit={schedule} className="appointment-form">
      {patient && <div className="ws-notice" role="status"><strong>{patient.name}</strong><p>{patient.email}</p><p>Only basic account information is shown until the patient accepts and pays.</p></div>}
      ...
      <div className="ws-actions"><button className="ws-link" disabled={busy || !enabled || !patient}>{busy ? "Scheduling…" : "Schedule consultation"}</button></div>
    </form>
